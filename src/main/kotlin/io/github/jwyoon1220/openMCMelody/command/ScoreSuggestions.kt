...
/** Tab-completion suggestion providers shared across [ScoreCommand]'s Brigadier tree. */
object ScoreSuggestions {
    ...
    fun scoreFiles(scoresFolder: File): SuggestionProvider<CommandSourceStack> = SuggestionProvider { _, builder ->
        ...
        val names = scoresFolder.listFiles { f -> f.isFile && SongFiles.isPlayable(f.name) }
            ?.map { it.name }?.sorted() ?: emptyList()
        names.filter { it.lowercase(Locale.ROOT).startsWith(remaining) }.forEach { builder.suggest(it) }
        builder.buildFuture()
    }

    fun playlistNames(playlistManager: PlaylistManager): SuggestionProvider<CommandSourceStack> = SuggestionProvider { _, builder ->
        val remaining = builder.remaining.lowercase(Locale.ROOT)
        playlistManager.list().filter { it.lowercase(Locale.ROOT).startsWith(remaining) }.forEach { builder.suggest(it) }
        builder.buildFuture()
    }

    fun soundPacks(soundPackManager: SoundPackManager): SuggestionProvider<CommandSourceStack> = SuggestionProvider { _, builder ->
        val remaining = builder.remaining.lowercase(Locale.ROOT)
        soundPackManager.listAvailable().filter { it.lowercase(Locale.ROOT).startsWith(remaining) }.forEach { builder.suggest(it) }
        builder.buildFuture()
    }

    fun audioFiles(audioFolder: File): SuggestionProvider<CommandSourceStack> = SuggestionProvider { _, builder ->
        val remaining = builder.remaining.lowercase(Locale.ROOT)
        val names = audioFolder.listFiles { f -> f.isFile && AudioFiles.isPlayable(f.name) }
            ?.map { it.name }?.sorted() ?: emptyList()
        names.filter { it.lowercase(Locale.ROOT).startsWith(remaining) }.forEach { builder.suggest(it) }
        builder.buildFuture()
    }
    ...
    fun soundFonts(soundfontsFolder: File): SuggestionProvider<CommandSourceStack> = SuggestionProvider { _, builder ->
        val remaining = builder.remaining.lowercase(Locale.ROOT)
        val names = soundfontsFolder.listFiles { f -> f.isFile && (f.name.endsWith(".sf2", ignoreCase = true) || f.name.endsWith(".dls", ignoreCase = true)) }
            ?.map { it.name }?.sorted() ?: emptyList()
        names.filter { it.lowercase(Locale.ROOT).startsWith(remaining) }.forEach { builder.suggest(it) }
        builder.buildFuture()
    }
